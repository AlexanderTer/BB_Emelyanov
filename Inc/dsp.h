...
typedef struct {
	...
	float k,k_boost, k_bb, k_buck;   	// Коэффициент игтегратора (включает период дискретизации)
...
typedef struct {
	...
	float k, k_boost, k_bb, k_buck; 	// Коэффициент дифференциатора (включает период дискретизации)
...
typedef struct {
	...
	float kp, kp_boost, kp_bb, kp_buck;			// Пропорциональная часть
